...
exists** (new 2026-09-09, roadmap item #16 — corrects this section's
...
| `test_optional_anthropic_import.py` | Module, real subprocess | New 2026-09-18 — regression test for a second, same-day real bug (see `ARCHIVE_WEB_STREAMLIT.md`): `vision_client.py`'s `anthropic` import is now guarded the same way, since `streamlit_app/requirements.txt` (a separately maintained file Streamlit Community Cloud prefers over `pyproject.toml`) had drifted out of sync and didn't list it. Same real-subprocess technique as `test_optional_tkinter_import.py`. |
| `test_vision_client.py` | Function | `extract_via_claude`'s happy path and its raise when Claude's response has no matching `tool_use` block. Signature changed 2026-09-09 (roadmap item #16): takes `image_bytes: bytes, media_type: str` rather than `image_path: Path` — both real callers (`main.py`'s `UploadFile`, Streamlit's `UploadedFile`) already have bytes in memory, so the old disk read (`image_path.read_bytes()`) was dropped rather than gaining a temp-file indirection. The three interactive callers (`truck_tag.py`/`trailer_tag.py`/`scale_ticket.py`) now compute bytes/media-type at their own call site via the new `vision_client.image_bytes_and_media_type()` helper. |
...
| `test_ocr_common.py` | Function | New 2026-09-09 (roadmap item #16). `get_ocr_backend()` — defaults to `"tesseract"` when `HDTTOOLS_OCR_BACKEND` is unset, reads an explicit `"tesseract"`/`"claude"` value, and raises `ValueError` (fail loud, no silent fallback) on anything else. |
| `test_ocr_output_key_contracts.py` | **Interface** | Two opposite-direction key-set checks per OCR module (2026-08-22): every `_parse_fields()` key must be a declared `schemas.py` field (the FastAPI `response_model`/`extra="ignore"` silent-drop risk), and every `fields.py`/`FIELDS` name must map to a real `_parse_fields()` key (the Streamlit `_extract_fields`'s `keep`-filter silent-drop risk, opposite direction since `FIELDS` deliberately shows only a curated subset of what OCR extracts). Both exclude the same known manual-only fields (`standalone_weight_lb`, `axle_count`). Calls `_parse_fields("")` — the full key set is structurally present regardless of match success, so no synthetic label text is needed, just the shape. Extended 2026-09-09 (roadmap item #16) with the same schema-direction check for the three new Claude-vision extractor functions — `extract_via_claude` mocked with a fake response shaped from each module's own `_SCHEMA` (`_fake_claude_fields()`), so the check tracks any future schema edit automatically rather than hand-duplicating field names a second time. |
| `test_claude_vision_external.py` | **External** | New 2026-09-09 (roadmap item #16) — the first External-tier test on this platform (see "Event-based tiers" above). `@pytest.mark.skipif`'d unless a real `ANTHROPIC_API_KEY` is set; one real Claude vision call per doc type against a random pass-pool image (`scripts/pass_pool.py`/`scripts/vehicle_discovery.py`, item #13's existing infra), asserting no *new* mismatch against `golden_fields.json`'s documented fields beyond what `known_ocr_limitations` already records (a deliberately looser tolerance than `test_pass_pool_regression.py`'s strict equality — see that file's own docstring for why). |
