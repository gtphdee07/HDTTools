...
**Decided, 2026-08-24**: the graphic goes in the repo `README.md` itself
(so it's visible the moment someone checks the repo out), broken out by
product line (web, Streamlit, Android, plus `scan-proxy` as its own
fourth line rather than folded into Android — it's a genuinely separate
deployable service), with per-line color coding (Blue 100% passing,
Green >90%, Yellow >80%, Red <80%) and a coverage percentage per line.
Since GitHub renders `README.md` as static markdown (no live JS at view
time), the graphic has to be a checked-in image regenerated by a script,
not a live dashboard — matches the original framing exactly.

**✅ Real terminology confusion found and fully unpacked while planning
this, 2026-08-24 — this is what actually paused the dashboard (see
`NEXT_STEPS.md` item #9).** Drafting the dashboard's own tier breakdown
("indicators for minor, major and integration testing") ran headfirst
into a real ambiguity this repo's own docs invite, even though they're
individually correct: there are **three genuinely distinct concepts**
that are easy to conflate as one.

1. **The original 2026-08-15 sketch** (Sanity → Module regression →
   Integration sanity → Full/"weekend" regression) — never built, fully
   superseded before any of it was implemented. Its "Integration sanity"
   name is the direct ancestor of the confusion — it's why "integration"
   felt like an established term when it was never actually built.
2. **Minor/Major** (decided 2026-08-20, what *actually* replaced #1) —
   not tiers at all, a two-way regression-*scoping* rule deciding *which
   test categories a diff calls for*: Minor (internal-only change) runs
   a module's function + interaction tests; Major (new library, or a
   parameter/return-shape/interface change) runs the module's full
   suite — all four categories (Function, Interaction, Module,
   **Interface** — not "Integration") — plus the other module's Major
   suite and inter-module interface tests if the changed interface is
   shared.
3. **Sanity/Daily/Weekly/Release (`scan-proxy`) and Unit/Daily/Weekly
   (Android)** — never replaced by #2, a completely orthogonal axis
   answering "how much real network dependency does this specific test
   have, and how often should that real-world exposure get re-verified,"
   a property of the test itself, independent of any diff. Explicitly
   documented as staying independent: "Turned out not to need a merge or
   a rewrite of either scheme — they're independent axes answering
   different questions."

Net: **there was no "Integration" tier or category actually established
anywhere in this repo** — the closest real things are "Interface"
(category 4, internal inter-module contracts) and the real-network
Weekly/Release tiers, and they're not the same thing.

**Decided, 2026-08-24, once this was untangled — a real nomenclature
redesign, not just a clarification.** The user's own framing: the
time-based tier names (`Daily`/`Weekly`) are "a digital design thing due
to multi-day runtimes" (verification-engineering background) that "don't
have much use in software" here — and indeed, `ARCHIVE_TESTING.md`
itself already documents that these tiers never ran on a real cadence in
practice ("just cheap enough to run anytime, ad hoc, no enforced
schedule"). Decided:

- **Retire Sanity/Daily/Weekly/Release and Unit/Daily/Weekly as concepts
  entirely** (not kept alongside the new model) — replaced by pure
  event-based triggers under Minor/Major, applied *consistently across
  all four platforms* (today only Android/`scan-proxy` have tier
  language at all; Web/Python never did).
- **New 5th test category: External** (peer to Function/Interaction/
  Module/Interface). Verifies a real 3rd-party boundary (RevenueCat,
  Anthropic) hasn't silently changed — a fundamentally different
  question than an internal module-to-module contract. Considered
  "Boundary" as the name instead; set aside since that word's already
  used informally for `TESTING.md` category 4's cheap-layer-first
  sequencing principle, and reusing it for the category name would
  re-introduce a different confusion of the same shape as the one just
  resolved. **"External" confirmed.**
- **Two trigger conditions for External**, not one: (a) diff-driven,
  same logic Interface already uses — a Major change touching
  ...
  explicit suspicion of drift. This is the piece a purely diff-driven
  model can't cover on its own: RevenueCat or Anthropic can change their
  own behavior with zero commits on our side.
- **Today's real Weekly-vs-Release *scope* distinction is preserved**,
  not flattened: Weekly-equivalent tests go through our own deployed
  service (does *our* integration still work); Release-equivalent tests
  hit the provider directly, bypassing our code (did *their* API itself
  change, isolated from our own bugs) — exactly the cheap/isolated-layer-
  first sequencing principle already in `TESTING.md`. Both become real
  suites *within* External (the same way "Module" already contains many
  files' worth of tests), not two separate named tiers.
- **Android's Unit-vs-instrumented(Daily) split stays, undisturbed** —
  judged orthogonal to this redesign, since it's a real runtime-
  environment fact (JVM vs. needs-a-real-Android-runtime/emulator), not
  a time-cadence fiction like "Weekly" was.
- **The coverage-gate release script and the whole event-based model
  apply to all four platforms uniformly, including Web and Streamlit** —
  explicit user correction to an earlier draft of this plan, which had
  proposed scoping the release-gate script to only Android/`scan-proxy`
  (the two platforms with a real release event *today*). Neither Web nor
  Streamlit has one yet (Streamlit Cloud auto-deploys from git push; Web
  has no hosting at all), but both are expected to gain one, and the
  design shouldn't special-case them out now just because that event
